...
export default function EditKategoriaShpenzimeveForm() {
   const { data: session } = useSession();
   //@ts-ignore
   const token = session?.token?.accessToken;
   // console.log();
   const router = useRouter();
   const { isOpen, kategoriaEShpenzimeveData, onClose } = useEditKategoriaShpenzimeveModal();
   ...
   const onSubmit = async (data: FormValues) => {
      ...
      try {
         if ( kategoriaEShpenzimeveData) {
            console.log(data);
            await updatekategoriaEShpenzimeve(data,  kategoriaEShpenzimeveData.id, token);
            toast.success("Kategoria e shpenzimeve u ndryshua me sukses");
            form.reset();
            onClose();
            router.refresh();
               ...
               render={({ field }) => (
                  <FormItem>
                     <FormControl>
                        <Input
                           id="emri" 
                           defaultValue={kategoriaEShpenzimeveData?.emri || field.value}
                           {...field}
            ...
            render={({ field }) => (
               <FormItem>
                  <FormControl>
                     <Textarea
                        id="pershkrimi"
                        defaultValue={kategoriaEShpenzimeveData?.pershkrimi || field.value}
                        rows={3}
                        {...field}
                     />
                  </FormControl>
